...
const units = {
  ...
  'bytes-per-second': 'Bytes per second (B/s)',
  'kilobytes-per-second': 'Kilobytes per second (KB/s)',
  'megabytes-per-second': 'Megabytes per second (MB/s)',
  'gigabytes-per-second': 'Gigabytes per second (GB/s)',
};

// All conversions relative to bits per second
const factors: { [key: string]: number } = {
  ...
  'bytes-per-second': 8,
  'kilobytes-per-second': 8 * 1000,
  'megabytes-per-second': 8 * 1_000_000,
  'gigabytes-per-second': 8 * 1_000_000_000,
};
...
export default function DataTransferSpeedConverter() {
  ...
  const handleExport = (format: 'txt' | 'csv') => {
    ...
    let content = '';
    const filename = `data-speed-conversion.${format}`;
    const { value, fromUnit, toUnit } = formData;

    if (format === 'txt') {
      content = `Data Transfer Speed Conversion\n\nInputs:\n- Value: ${value}\n- From: ${fromUnit}\n- To: ${toUnit}\n\nResult:\n- Converted Value: ${result}`;
    } else {
