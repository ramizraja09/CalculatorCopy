...
// This component is identical to CalorieCalculator as TDEE is the core of calorie needs.
// For a real app, you might differentiate them or explain the terms more clearly.
...
export default function TdeeCalculator() {
  ...
  const calculateTdee = (data: FormData) => {
  ...
  return (
    <form onSubmit={handleSubmit(calculateTdee)} className="grid md:grid-cols-2 gap-8">
      {/* Inputs */}
        ...
        {results ? (
            <Card>
                <CardContent className="p-6 text-center">
                    ...
                    <p className="text-4xl font-bold my-2">{Math.round(results.tdee)}</p>
                    <p className="text-muted-foreground">calories per day</p>
                </CardContent>
            </Card>
        ) : (
             <div className="flex items-center justify-center h-40 bg-muted/50 rounded-lg border border-dashed"><p>Enter details to calculate your TDEE</p></div>
        )}
      </div>
    </form>
  );
